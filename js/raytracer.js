define(['webgl', 'window'], function raytracer(WebGLDebugUtils, window) {
  ...
  class RayTracer {
    ...
    constructor($canvas) {
      ...
      //this.initShaders();
      var vsFile  = "shaders/vertex.glsl";
      var fsFile = "shaders/fragment.glsl";
      this.initShaderFile(vsFile, fsFile);
    }
    ...
    initShaderFile(vsFile, fsFile) {
      ...
      var onShaderLoaded = function () {
        if (vs && fs) {
          ...
          var vertexShader = _this.getShaderContent(vs,  _this.gl.VERTEX_SHADER);
          var fragmentShader = _this.getShaderContent(fs, _this.gl.FRAGMENT_SHADER);
          _this.initShaders(vertexShader, fragmentShader);
        }
      };
    ...
    loadShaderFromFile(filename, onLoadShader) {
    ...
    getShaderContent(shaderSrc,type){
